...
namespace AlexanderGropp\Component\BlaulichtMonitor\Administrator\View\Organisation;
...
/**
 * View-Klasse für die Einzelansicht einer Organisation.
 * Stellt das Formular dar und konfiguriert die Toolbar für die Bearbeitung.
 */
class HtmlView extends BaseHtmlView
{
	/** @var \JForm Formularobjekt für die Organisation */
	public $form;
	...
	/**
	 * Anzeige der Einzelansicht (Formular) für eine Organisation.
	...
	{
		// Lade Formular, Status und Organisation-Daten aus dem Model
	...
	/**
	 * Konfiguriert die Toolbar für die Organisation-Bearbeitung.
	...
	protected function addToolbar()
	{
		...
		ToolbarHelper::title(
			Text::_('COM_BLAULICHTMONITOR_ORGANISATION_TITLE_' . ($isNew ? 'ADD' : 'EDIT'))
		);

		// Zeige die Buttons zum Speichern und Anwenden, falls Berechtigung vorhanden
		if ($canDo->get('core.create')) {
			$toolbar->apply('organisation.apply');
			$toolbar->save('organisation.save');
		}

		// Button zum Abbrechen/Schließen der Bearbeitung
		$toolbar->cancel('organisation.cancel', 'JTOOLBAR_CLOSE');
	}
}
