...
/**
 * Model für die Einheiten-Übersicht im Backend.
 ...
 */
class EinheitenModel extends ListModel
{
	...
	/**
	 * Baut die SQL-Abfrage für die Einsatzarten-Liste.
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					...
					$db->quoteName('a.name'),
					$db->quoteName('a.url'),
					$db->quoteName('a.standort_title'),
					$db->quoteName('a.standort_strasse'),
					$db->quoteName('a.standort_hausnummer'),
					$db->quoteName('a.standort_plz'),
					$db->quoteName('a.standort_ort'),
					$db->quoteName('a.organisation_id'),
		...
		)->from($db->quoteName('#__blaulichtmonitor_einheiten', 'a'))
			// Join für created_by
	...
	/**
	 * Lädt das Filterformular (siehe filter_einheiten.xml).
	 * Wird im View verwendet, um das Such- und Sortierformular anzuzeigen.
	 */
	public function getFilterForm($data = [], $loadData = true)
	{
		return $this->loadForm(
			'com_blaulichtmonitor.einheiten',
			'filter_einheiten',
