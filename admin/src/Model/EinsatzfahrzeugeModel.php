...
/**
 * Model für die AEinsatzarten-Übersicht im Backend.
 ...
 */
class EinsatzfahrzeugeModel extends ListModel
{
	...
	protected function getListQuery()
	{
		...
		$query->select(
			$this->getState(
				...
				[
					$db->quoteName('a.id'),
					$db->quoteName('a.funkrufname'),
					$db->quoteName('a.einheit_id'),
					$db->quoteName('a.beschreibung'),
					$db->quoteName('a.bild_url'),
					$db->quoteName('a.url'),
					$db->quoteName('a.in_dienst'),
	...
	/**
	 * Lädt das Filterformular (siehe filter_einsatzarten.xml).
	 * Wird im View verwendet, um das Such- und Sortierformular anzuzeigen.
	 */
	public function getFilterForm($data = [], $loadData = true)
	{
		return $this->loadForm(
			'com_blaulichtmonitor.einsatzfahrzeuge',
			'filter_einsatzfahrzeuge',
